...
void ShowProcs(HWND listBox)
{
	...
	do
	{
		processes[process_count].dwSize = sizeof(PROCESSENTRY32);
		result = Process32Next(toolHelp, &processes[process_count]);
		process_count++;
	} while (result);
	...
	for (int i = 0; i < process_count; i++) {
		SendMessage(listBox, LB_ADDSTRING, 0, (LPARAM)processes[i].szExeFile);
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		SetWindowPos(hWnd, nullptr, 300, 100, 650, 600, SWP_SHOWWINDOW);
		...
		case 101:
			if (HIWORD(wParam) == LBN_SELCHANGE)
			{
				...
				if (itemId != -1) {
					...
					GetModules(processes[itemId].th32ProcessID, listBox2);
				}
			}
			break;
		case IDLE:
	...
	case WM_CONTEXTMENU:
		if ((HWND)wParam == listBox1) {
			...
			if (itemId != -1) {
				...
				AppendMenu(hMenu, pC == IDLE_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, IDLE, L"Idle");
				AppendMenu(hMenu, pC == NORMAL_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, NORMAL, L"Normal");
				AppendMenu(hMenu, pC == HIGH_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, HIGH, L"High");
				AppendMenu(hMenu, pC == REALTIME_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, REALTIME, L"Real time");
